...
let hugLevel = 0;
const extrasPhoneViewport = window.matchMedia("(max-width: 760px)");
const extrasReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
...
if (skyCanvas) {
  ...
  const starCount = extrasPhoneViewport.matches || extrasReducedMotion.matches ? 52 : 120;
  let skyAnimationFrame = 0;
  let skyIsVisible = true;

  function sizeSkyCanvas() {
    const rect = skyCanvas.getBoundingClientRect();
    const pixelRatio = extrasPhoneViewport.matches ? 1 : Math.min(window.devicePixelRatio || 1, 1.5);
    skyCanvas.width = Math.max(320, Math.round(rect.width * pixelRatio));
    skyCanvas.height = Math.max(260, Math.round(rect.height * pixelRatio));
  }
  ...
  function drawSky(time = 0) {
    const animateSky = !(extrasPhoneViewport.matches || extrasReducedMotion.matches);
    if (document.hidden || !skyIsVisible) {
      if (animateSky) skyAnimationFrame = requestAnimationFrame(drawSky);
      return;
    }
  ...
  if ("IntersectionObserver" in window) {
    const skyObserver = new IntersectionObserver(
      ([entry]) => {
        skyIsVisible = entry.isIntersecting;
      },
      { threshold: 0.05 },
    );
    skyObserver.observe(skyCanvas);
  }

  window.addEventListener("resize", sizeSkyCanvas, { passive: true });
  if (extrasPhoneViewport.matches || extrasReducedMotion.matches) {
    drawSky(0);
  } else {
    skyAnimationFrame = requestAnimationFrame(drawSky);
  }
}
