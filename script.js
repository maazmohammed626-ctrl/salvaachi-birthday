...
const mobileEssentialStops = new Set([
  "garden", "cake", "wishes", "distance", "memories", "playlist", "promises", "letters", "hug", "stars",
  "photos", "moon-note", "open-when", "proud", "compliments", "love-jar", "secret-letter", "queen",
  "dream-lanterns", "my-girl", "treasure-map", "voice-love", "art-studio", "sweet-spicy", "good-stars",
  "kiss-delivery", "phone-signal", "flower-rain", "bookstore-date", "aurora-sky", "firefly-garden",
  "shooting-letter", "planet-orbit", "architect-dream", "chest-comfort", "reader-night", "trophy-wall",
  "rare-soul", "love-weather", "care-menu", "final-fireworks", "notes", "ninety-love-map", "marry-soon",
  "ring-box", "vow-garden", "forever-home", "old-us", "dream-city", "hundred-reasons", "final-blessing",
]);

function phoneSafeCount(desktopCount, phoneCount) {
  return phoneViewport.matches || reducedMotion.matches ? phoneCount : desktopCount;
}
...
const unlockWelcome = document.querySelector("#unlock-welcome");
const welcomeClose = document.querySelector("#welcome-close");
const saveMomentButton = document.querySelector("#save-moment-button");
const saveMomentNote = document.querySelector("#save-moment-note");
...
function setupLoveLock() {
  ...
  if (saved) {
    ...
    document.body.classList.add("has-unlocked");
    return;
  }
...
function setupMobileSafeMode() {
  if (!phoneViewport.matches) return;

  document.body.classList.add("mobile-safe-mode");
  const sections = [...document.querySelectorAll("main > section")];

  sections.forEach((section, index) => {
    const keepVisible = index === 0 || !section.id || mobileEssentialStops.has(section.id);
    section.classList.toggle("mobile-lite-hidden", !keepVisible);
  });

  storyLinks.forEach((link) => {
    const targetId = link.getAttribute("href")?.replace("#", "");
    link.classList.toggle("mobile-lite-hidden", Boolean(targetId && !mobileEssentialStops.has(targetId)));
  });

  const storyNav = document.querySelector(".story-steps");
  if (!storyNav || document.querySelector(".mobile-safe-note")) return;

  const note = document.createElement("div");
  note.className = "mobile-safe-note";
  note.innerHTML = `
    <span>Made gentle for your phone, Salvuu.</span>
    <button type="button">Show all stops</button>
  `;
  storyNav.insertAdjacentElement("afterend", note);
  note.querySelector("button").addEventListener("click", () => {
    document.body.classList.remove("mobile-safe-mode");
    document.body.classList.add("mobile-full-mode");
    document.querySelectorAll(".mobile-lite-hidden").forEach((item) => item.classList.remove("mobile-lite-hidden"));
    note.remove();
  });
}
...
function unlockLoveSite(event) {
  ...
  if (value === "MIHRUSALVU") {
    ...
    document.body.classList.remove("is-locked");
    document.body.classList.add("has-unlocked");
    unlockWelcome?.classList.add("is-visible");
    launchWindowConfetti();
  } else {
    lockError.textContent = "That key did not open it. Try the private code Mihruu sent you.";
    loveLock.classList.remove("is-wrong");
    window.setTimeout(() => loveLock.classList.add("is-wrong"), 20);
  }
}

function closeUnlockWelcome() {
  unlockWelcome?.classList.remove("is-visible");
}

function saveFinalMoment() {
  saveMomentNote.textContent = "Screenshot this ending and send it to Mihruu. He will be smiling so much knowing you reached the hundredth stop.";
  saveMomentNote.classList.add("is-visible");
  launchWindowConfetti();
}
let decreeIndex = 0;
let girlIndex = 0;
...
function launchWindowConfetti() {
  ...
  for (let index = 0; index < phoneSafeCount(180, 62); index += 1) {
...
function launchLoveFireworks() {
  ...
  const burstCount = phoneSafeCount(12, 5);
  const piecesPerBurst = phoneSafeCount(48, 20);
  for (let burst = 0; burst < burstCount; burst += 1) {
    ...
    for (let index = 0; index < piecesPerBurst; index += 1) {
      const piece = document.createElement("span");
      ...
      const angle = (Math.PI * 2 * index) / piecesPerBurst;
      const distance = phoneSafeCount(95 + Math.random() * 220, 70 + Math.random() * 130);
...
function startFlowerRainLegacy() {
  ...
  for (let index = 0; index < phoneSafeCount(70, 26); index += 1) {
...
function startFlowerRain() {
  ...
  for (let index = 0; index < phoneSafeCount(76, 30); index += 1) {
    const petal = document.createElement("span");
  ...
  window.setTimeout(() => {
    windowConfetti.innerHTML = "";
  }, phoneSafeCount(6200, 3900));
}

function createTinyBurst(source) {
  const rect = source.getBoundingClientRect();
  for (let index = 0; index < phoneSafeCount(12, 6); index += 1) {
    const piece = document.createElement("span");
...
function startComfortHeartbeat() {
  ...
  comfortNote.textContent = comfortHeart.classList.contains("is-beating")
    ? "Chest warmth on: Salvuu can rest here, safe, loved, and close to Mihruu."
    : "Chest warmth paused, but Mihruu is still right here.";
}
...
function setActiveStoryStep(id) {
  ...
  if (phoneViewport.matches && currentTrack && id !== "cake") {
    return;
  }
...
welcomeClose?.addEventListener("click", closeUnlockWelcome);
saveMomentButton?.addEventListener("click", saveFinalMoment);
setupLoveLock();
setupImageFallbacks();
